...
def __conv2d_p(name, x, w=None, num_filters=16, kernel_size=(3, 3), padding='SAME', stride=(1, 1),
    ...
    :param initializer: (tf.contrib initializer) The initialization scheme, He et al.
           normal or Xavier normal are recommended.
    ...
    with tf.variable_scope(name) as scope:
        if dilation_factor > 1:
            conv_o_b = __depthwise_conv2d_atrous_p(name=scope, x=x, w=w, kernel_size=kernel_size, padding=padding,
                                                   ...
                                                   bias=bias, dilation_factor=dilation_factor)
        else:
...
def depthwise_separable_atrous_conv2d(name, x, w_depthwise=None, w_pointwise=None, width_multiplier=1.0, num_filters=16,
                                      kernel_size=(3, 3), dilation_factor=1, padding='SAME', stride=(1, 1),
                                      initializer=tf.contrib.layers.xavier_initializer(), l2_strength=0.0,
                                      biases=(0.0, 0.0), activation=None, batchnorm_enabled=True, is_training=True):
    total_num_filters = int(round(num_filters * width_multiplier))
    with tf.variable_scope(name):
        conv_a = depthwise_conv2d('depthwise', x=x, w=w_depthwise, kernel_size=kernel_size, padding=padding,
...
def shufflenet_unit(name, x, w=None, num_groups=1, group_conv_bottleneck=True, num_filters=16, stride=(1, 1),
    ...
    with tf.variable_scope(name):
        ...
        if group_conv_bottleneck:
            bottleneck = grouped_conv2d('Gbottleneck', x=x, w=w, num_filters=bottleneck_filters, kernel_size=(1, 1),
                                        padding='VALID',
            ...
            bottleneck = conv2d('bottleneck', x=x, w=w, num_filters=bottleneck_filters, kernel_size=(1, 1),
        ...
        if dilation > 1:
            depthwise = depthwise_conv2d('depthwise', x=shuffled, w=w, stride=stride, l2_strength=l2_strength,
                                         padding='SAME', bias=bias, dilation_factor=dilation, activation=None,
                                         batchnorm_enabled=batchnorm_enabled, is_training=is_training)
        else:
            padded = tf.pad(shuffled, [[0, 0], [1, 1], [1, 1], [0, 0]], "CONSTANT")
            depthwise = depthwise_conv2d('depthwise', x=padded, w=w, stride=stride, l2_strength=l2_strength,
